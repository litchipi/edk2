...
#include <Library/AArch32/StandaloneMmCoreEntryPoint.h>
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if (FfaEnabled) {
      EventCompleteSvcArgs->Arg0 = ARM_SVC_ID_FFA_MSG_SEND_DIRECT_RESP_AARCH32;
      EventCompleteSvcArgs->Arg1 = 0;
      EventCompleteSvcArgs->Arg2 = 0;
      EventCompleteSvcArgs->Arg3 = ARM_SVC_ID_SP_EVENT_COMPLETE_AARCH32;
      EventCompleteSvcArgs->Arg4 = SvcStatus;
    } else {
      EventCompleteSvcArgs->Arg0 = ARM_SVC_ID_SP_EVENT_COMPLETE_AARCH32;
...
GetSpmVersion (VOID)
{
  ...
  ArmCallSvc (&SpmVersionArgs);
    ...
    DEBUG ((DEBUG_INFO, "SVC Version: a0=%x  a1=%x  a2=%x\n",
	    SpmVersionArgs.Arg0, SpmVersionArgs.Arg1, SpmVersionArgs.Arg2));
...
{
  if (FeaturePcdGet (PcdFfaEnable)) {
    InitMmFoundationSvcArgs->Arg0 = ARM_SVC_ID_FFA_MSG_SEND_DIRECT_RESP_AARCH32;
    InitMmFoundationSvcArgs->Arg1 = 0;
    InitMmFoundationSvcArgs->Arg2 = 0;
    InitMmFoundationSvcArgs->Arg3 = ARM_SVC_ID_SP_EVENT_COMPLETE_AARCH32;
    InitMmFoundationSvcArgs->Arg4 = *Status;
  } else {
    InitMmFoundationSvcArgs->Arg0 = ARM_SVC_ID_SP_EVENT_COMPLETE_AARCH32;
